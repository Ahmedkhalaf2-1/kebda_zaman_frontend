...
class _FakeAdminOrderNotificationRepository
...
void main() {
  // `AdminOrderNotificationNotifier` now polls in the background
  // (`PollingNotifierMixin`, added alongside the admin order-alert bell),
  // which registers a `WidgetsBindingObserver` on build — needs a real
  // binding even though this test never pumps a widget.
  TestWidgetsFlutterBinding.ensureInitialized();

  group('NotificationService.handleAdminNewOrderRefresh', () {
    test(
      'a NEW_ORDER message re-fetches the admin unread count and notification list',
      () async {
        final fakeRepo = _FakeAdminOrderNotificationRepository();
        final container = ProviderContainer(
          overrides: [
            adminOrderNotificationRepositoryProvider.overrideWithValue(
              fakeRepo,
            ),
          ],
        );
        ...
        final sub2 = container.listen(
          adminUnreadNotificationCountProvider,
          (_, __) {},
        );
        ...
        NotificationService.instance.handleAdminNewOrderRefresh(
          _payload({
            'type': 'NEW_ORDER',
            'notificationId': 'n1',
            'orderId': 'order-1',
    ...
    test(
      'a non-NEW_ORDER message does not touch the admin notification providers',
      () async {
        final fakeRepo = _FakeAdminOrderNotificationRepository();
        final container = ProviderContainer(
          overrides: [
        ...
        NotificationService.instance.attachProviderContainer(container);
        NotificationService.instance.handleAdminNewOrderRefresh(
          _payload({'type': 'order_created', 'entityId': 'order-1'}),
        );
